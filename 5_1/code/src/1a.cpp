...
void unset_bit(unsigned char& x, unsigned char bit);

int main(void) {
    using std::array, std::bitset;
    using std::cout, std::cin, std::endl;

    array<unsigned char, 3> arr {255, 19, 24}; // массив с тестовыми числами

    for (auto& el : arr) {
        cout << "до: " << bitset<8>{el} << " = " << (unsigned int)el << endl;
        unset_bit(el, 5); // устанавливаем 5 бит в 0
...
void unset_bit(unsigned char& x, unsigned char bit) {
    ...
    x &= ~mask; // инвертируем маску и применяем логическое `И`, в результате получаем то же самое число, но с искомым битом, установленным в ноль
}
